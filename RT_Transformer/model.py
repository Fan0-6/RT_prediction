...
class Tester(object):
    ...
    #     return mae.item(), medAE.item(), mre.item(), rmse.item(), r2.item()
    #     # return mae
    def test_regressor(self, data_loader):
        y_true_list = []
        y_pred_list = []
        ...
        # Compute metrics using the concatenated tensors
        mae = torch.abs(y_true_tensor - y_pred_tensor).sum()
        mre = torch.div(torch.abs(y_true_tensor - y_pred_tensor), y_true_tensor).mean()
        medAE = torch.median(torch.abs(y_true_tensor - y_pred_tensor))
        medRE = torch.median(torch.div(torch.abs(y_true_tensor - y_pred_tensor), y_true_tensor))
        rmse = torch.sqrt(torch.mean((y_true_tensor - y_pred_tensor) ** 2))
        score = torchmetrics.R2Score().to(self.device)
        r2 = score(y_pred_tensor, y_true_tensor)

        # Convert tensors to NumPy arrays for saving
        y_true_array = y_true_tensor.cpu().numpy()
        y_pred_array = y_pred_tensor.cpu().numpy()

        return mae.item(), medAE.item(), mre.item(), rmse.item(), r2.item(), y_true_array, y_pred_array
